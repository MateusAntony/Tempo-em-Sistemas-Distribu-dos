...
ip_3 = os.getenv('ip3', endereco_ip)
relogios = {'2': f"http://{ip_2}:8989", '3': f"http://{ip_3}:4378"}

lider = None
eleicao_ativa = False
lock = threading.Lock()

url_local=f"http://{endereco_ip}:5656"
...
def informar_novo_lider(novo_lider):
    ...
    for id, url in relogios.items():
        try:
            ...
            print(f"Erro ao informar novo líder para {id}")
            continue
    ...
    while True:
        ...
        if lider is not None:
            try:
                ...
                print(f"Erro ao verificar tempos dos relógios: {e}")
                eleger_e_informar()
        else:
            eleger_e_informar()
...
def eleicao():
    ...
    for id, url in relogios.items():
        try:
            ...
            print(f"Erro ao obter tempo de {id}")
            continue
    lider = novo_lider
...
if __name__ == '__main__':
    ...
    app.run(debug=True, host=endereco_ip, port=5656)
